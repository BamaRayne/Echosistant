...
// Call using: setHueColor("Cool White", hueBulbList)
def setHueColor(colorName, hueBulbs) {    
    for (color in colors()) {
		if (color.name == colorName) {
        	int hueVal = Math.round(color.h / 3.6)
			def hueSet = [hue: hueVal, saturation: color.s, level: color.l]
            hueBulbs.setColor(hueSet)
		}
	}
}
